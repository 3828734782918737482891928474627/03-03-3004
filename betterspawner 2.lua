...
local EquippedPet2 = nil  -- Alt slot (certificate / second pet)
...
local function UnequipPetAlt(petData)
    local pet = SpawnedPets[petData.unique]
    if not pet or not pet.model then return end
    ...
    if EquippedPet2 and EquippedPet2.unique == petData.unique then
        EquippedPet2 = nil
    end
...
local function EquipPetAlt(petData)
    if petData.category ~= "pets" then return end

    -- Only unequip alt slot, leave slot 1 (EquippedPet) alone
    if EquippedPet2 then
        UnequipPetAlt(EquippedPet2)
    end
    ...
    elseif endpoint == "ToolAPI/Unequip" then
        return {
            InvokeServer = function(_, uniqueId)
                ...
                -- Unequip from whichever slot this pet is in
                if EquippedPet2 and EquippedPet2.unique == uniqueId then
                    UnequipPetAlt(pet.data)
                else
                    UnequipPet(pet.data)
                end
...
-- ── Dual Pet Equip Section ─────────────────────────────────────────────────

local dualLabel = Instance.new("TextLabel")
dualLabel.Size = UDim2.new(1, 0, 0, 10)
dualLabel.Position = UDim2.new(0, 0, 0, 138)
dualLabel.BackgroundTransparency = 1
dualLabel.Text = "🐾 Dual Pet Equip (client-only)"
dualLabel.Font = Enum.Font.Gotham
dualLabel.TextSize = 7
dualLabel.TextColor3 = Color3.fromRGB(160, 170, 200)
dualLabel.TextXAlignment = Enum.TextXAlignment.Left
dualLabel.Parent = toolsPanel

-- Slot 1 status
local slot1Label = Instance.new("TextLabel")
slot1Label.Size = UDim2.new(1, 0, 0, 10)
slot1Label.Position = UDim2.new(0, 0, 0, 150)
slot1Label.BackgroundTransparency = 1
slot1Label.Text = "📦 Slot 1: none"
slot1Label.Font = Enum.Font.Gotham
slot1Label.TextSize = 7
slot1Label.TextColor3 = Color3.fromRGB(120, 200, 120)
slot1Label.TextXAlignment = Enum.TextXAlignment.Left
slot1Label.Parent = toolsPanel

-- Slot 2 status
local slot2Label = Instance.new("TextLabel")
slot2Label.Size = UDim2.new(1, 0, 0, 10)
slot2Label.Position = UDim2.new(0, 0, 0, 161)
slot2Label.BackgroundTransparency = 1
slot2Label.Text = "📜 Slot 2: none"
slot2Label.Font = Enum.Font.Gotham
slot2Label.TextSize = 7
slot2Label.TextColor3 = Color3.fromRGB(180, 140, 255)
slot2Label.TextXAlignment = Enum.TextXAlignment.Left
slot2Label.Parent = toolsPanel

local function UpdateSlotLabels()
    slot1Label.Text = "📦 Slot 1: " .. (EquippedPet and (EquippedPet.properties.rp_name or EquippedPet.id) or "none")
    slot2Label.Text = "📜 Slot 2: " .. (EquippedPet2 and (EquippedPet2.properties.rp_name or EquippedPet2.id) or "none")
end

-- Equip Alt button (equips currently-named pet into slot 2)
local equipAltButton = Instance.new("TextButton")
equipAltButton.Size = UDim2.new(1, 0, 0, 22)
equipAltButton.Position = UDim2.new(0, 0, 0, 174)
equipAltButton.Text = "📜 Equip Alt (Slot 2)"
equipAltButton.Font = Enum.Font.GothamBold
equipAltButton.TextSize = 8
equipAltButton.BackgroundColor3 = Color3.fromRGB(100, 60, 180)
equipAltButton.TextColor3 = Color3.fromRGB(255, 255, 255)
equipAltButton.Parent = toolsPanel
Instance.new("UICorner", equipAltButton).CornerRadius = UDim.new(0, 6)

local equipAltStroke = Instance.new("UIStroke")
equipAltStroke.Color = Color3.fromRGB(180, 120, 255)
equipAltStroke.Thickness = 1.5
equipAltStroke.Transparency = 0.3
equipAltStroke.Parent = equipAltButton

-- Unequip Alt button
local unequipAltButton = Instance.new("TextButton")
unequipAltButton.Size = UDim2.new(1, 0, 0, 20)
unequipAltButton.Position = UDim2.new(0, 0, 0, 199)
unequipAltButton.Text = "❌ Unequip Slot 2"
unequipAltButton.Font = Enum.Font.GothamBold
unequipAltButton.TextSize = 8
unequipAltButton.BackgroundColor3 = Color3.fromRGB(120, 50, 50)
unequipAltButton.TextColor3 = Color3.fromRGB(255, 255, 255)
unequipAltButton.Parent = toolsPanel
Instance.new("UICorner", unequipAltButton).CornerRadius = UDim.new(0, 6)

equipAltButton.MouseButton1Click:Connect(function()
    -- Use whatever pet name is currently typed in the spawn panel
    local petName = nameInput.Text
    if petName == "" then
        equipAltButton.Text = "⚠ Type a pet name first!"
        task.wait(1.2)
        equipAltButton.Text = "📜 Equip Alt (Slot 2)"
        return
    end
    ...
    if not petId then
        equipAltButton.Text = "⚠ Pet not found!"
        task.wait(1.2)
        equipAltButton.Text = "📜 Equip Alt (Slot 2)"
        return
    end

    -- Find an existing spawned instance of this pet, or create a temporary one
    local targetPet = nil
    for uid, pet in pairs(SpawnedPets) do
        if pet.data and pet.data.id == petId then
            -- Prefer one that isn't already in slot 1
            if not (EquippedPet and EquippedPet.unique == uid) then
                targetPet = pet.data
                break
            end
        end
    ...
    if not targetPet then
        -- Spawn a fresh one specifically for slot 2
        local options = {
            mega_neon = flagState["M"],
            neon = flagState["N"],
            flyable = flagState["F"],
            rideable = flagState["R"],
            age = ({1,2,3,4,5,6})[currentAge],
            trick_level = 5,
            ailments_completed = 0,
            rp_name = GenerateUniquePetName()
        }
        targetPet = CreateInventoryItem(petId, "pets", options)
    end

    if targetPet then
        EquipPetAlt(targetPet)
        equipAltButton.Text = "✓ Slot 2 equipped!"
        UpdateSlotLabels()
        task.wait(0.8)
        equipAltButton.Text = "📜 Equip Alt (Slot 2)"
    end
...
unequipAltButton.MouseButton1Click:Connect(function()
    if EquippedPet2 then
        UnequipPetAlt(EquippedPet2)
        UpdateSlotLabels()
        unequipAltButton.Text = "✓ Slot 2 cleared!"
        task.wait(0.8)
        unequipAltButton.Text = "❌ Unequip Slot 2"
    end
end)

-- Keep slot labels live while Tools tab is open
task.spawn(function()
    while true do
        task.wait(1)
